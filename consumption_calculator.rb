...
class ConsumptionCalculator
  ...
  def fill_before_period
    ...
    first_day_with_value = first_date_with_value.strftime("%d").to_i
    return if first_day_with_value == 1

    if Date.parse(first_two_data_items[0][0]).month == Date.parse(first_two_data_items[1][0]).month
      last_date_of_month = Date.new(first_date_with_value.year, first_date_with_value.month, -1)
      last_date_of_month_consumption_value = data[ last_date_of_month.strftime("%Y-%m-%d") ]
      last_day_of_month = last_date_of_month.strftime("%d").to_i
      days_count = last_day_of_month - first_day_with_value
      day_consumption = (last_date_of_month_consumption_value - first_two_data_items[0][1]) / days_count
    else
